...
class MCMCMultGPU:
    def __init__(
        ...
        self._chain_states = tf.Variable(
            tf.tile(tf.expand_dims(initial_state, axis=0), [n_chains, 1]),
            dtype=tf.float32,
        )

        # boundaries
        self._upper_bounds = tf.convert_to_tensor(
            self._interface.scale_data(
                self._interface.chain.upper_bounds[:-1].reshape(1, -1)
            ),
            dtype=tf.float32,
        )
        self._lower_bounds = tf.convert_to_tensor(
            self._interface.scale_data(
                self._interface.chain.lower_bounds[:-1].reshape(1, -1)
            ),
            dtype=tf.float32,
        )
        ...
        self._matrix_handler = CovarianceUpdaterGPU(
            self._n_dim, update_step, max_update
        )

        # Calculate likelihoods for all chains (with GPU processing)
        self._current_loglikelihoods = tf.Variable(
            self._calc_likelihood(self._chain_states)
        )
    ...
    def _get_circular_indices(self, circular_params: List[str]):
        ...
        return [
            self._interface.chain.plot_branches.index(param)
    ...
    def _estimate_batch_size(self):
        ...
        print(
            f"Automatically determined batch size for the process: {estimated_batch_size}"
        )
    ...
    def propose_step_gpu(self):
        ...
        proposed_states = (
            self._matrix_handler.sample(self._n_chains) + self._chain_states
        )
        ...
        def apply_circular_bounds(idx):
            ...
            adjusted_values = lower_bound + tf.math.mod(
                proposed_states[:, idx] - upper_bound, upper_bound - lower_bound
            )
        ...
        proposed_states = tf.where(
            proposed_states < self._lower_bounds, self._chain_states, proposed_states
        )
        proposed_states = tf.where(
            proposed_states > self._upper_bounds, self._chain_states, proposed_states
        )
        ...
        log_diff = (
            proposed_loglikelihoods - self._current_loglikelihoods
        )  # Because network scales itself
        ...
        self._current_loglikelihoods.assign(
            tf.where(accept, proposed_loglikelihoods, self._current_loglikelihoods)
        )
    ...
    def save_mcmc_chain_to_pdf(
        self, filename: str, output_pdf: str, thinning: int = 10, burnin: int = 10000
    ):
        ...
        rescaled_chain = [
            self._interface.invert_scaling(chain[1000:, i])
        ...
        with PdfPages(output_pdf) as pdf:
            ...
            for i in tqdm(range(n_params)):
                ...
                ax.set_title(
                    f"Parameter {self._interface.chain.plot_branches[i]} MCMC Chain"
                )
                ax.set_xlabel("Step")
        ...
        with PdfPages(f"posterior_{output_pdf}") as pdf:
            ...
            for i in tqdm(range(n_params)):
                ...
                ax.hist(
                    self._interface.test_data.iloc[10000:, i].to_numpy(),
                    color="r",
                    label="Real Result",
                    alpha=0.3,
                    bins=bins,
        ...
        with PdfPages(f"ac_{output_pdf}") as pdf:
            for i in tqdm(range(n_params)):
                ...
                # for n, r in enumerate(rescaled_chain):
                ac = sm.tsa.acf(
                    rescaled_chain[0][:, i], nlags=len(rescaled_chain[0][:, 1])
                )
    ...
    def __call__(self, n_steps, output_file_name: str):
        ...
        with h5py.File(output_file_name, "w") as f:
            ...
            if "chain" in f:
                del f[
                    "chain"
                ]  # Delete if it already exists to avoid appending duplicate data

            self._dataset = f.create_dataset(
                "chain", (n_steps, self._n_chains, self._n_dim), chunks=True
            )
