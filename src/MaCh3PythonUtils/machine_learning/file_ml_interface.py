...
class FileMLInterface(ABC):
    ...
    def invert_label_scaling(self, input_labels):
        return self._label_scaler.inverse_transform(input_labels)
    ...
    def model_predict_unscale(self, testing_data: pd.DataFrame):
        pred = self.model_predict(testing_data)
        return self._label_scaler.inverse_transform(pred.reshape(-1,1))
