...
class NormalizingFlow:
    ...
    def _create_normalizing_flow(self):
        bijectors = [
            self._create_autoregressive_bijector() for _ in range(self._n_bijectors)
        ]
    ...
    def __call__(self):
        ...
        base_distribution = tfd.MultivariateNormalDiag(
            loc=tf.zeros(self._input_dim), scale_diag=tf.ones(self._input_dim)
        )
...
class TfNormalizingFlowModel(TfManualInterface):
    def build_model(self, **kwargs):
        ...
        self._model = NormalizingFlow(
            kwargs.get("hidden_units", [100]), kwargs.get("n_bijectors", 1), input_dim
        )()
    ...
    def train_model(self):
        ...
        scaled_data = tf.data.Dataset.from_tensor_slices(
            self.scale_data(self._training_data)
        ).batch(batch_size)
    ...
    def test_model(self):
        ...
        with PdfPages("likelihood_free_inference.pdf") as pdf:
            for i in range(self._chain.ndim - 1):
                sns.histplot(
                    surrogate_samples[:, i], label="Surrogate", color="blue", fill=False
                )
                sns.histplot(
                    self._chain.ttree_array[:, i],
                    label="Test Data",
                    color="blue",
                    fill=False,
                )
