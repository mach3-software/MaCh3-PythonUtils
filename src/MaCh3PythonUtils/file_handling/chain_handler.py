...
class ChainHandler:
    ...
    def __init__(
        self, file_name: str, ttree_name: str = "posteriors", verbose=False
    ) -> None:
        """_summary_
        ...
        try:
            ...
            raise IOError(
                f"The file '{file_name}' does not exist or does not contain '{ttree_name}"
            )

        print(f"Succesfully opened {file_name}:{ttree_name}")
        warnings.filterwarnings(
            "ignore", category=DeprecationWarning
        )  # Some imports are a little older
        warnings.filterwarnings(
            "ignore", category=UserWarning
        )  # Some imports are a little older
        warnings.filterwarnings(
            "ignore", category=pd.errors.PerformanceWarning
        )  # Not a fan of being yelled at by pandas

        self._plotting_branches = []  # Filled with branches we want to plot
        self._cuts = (
            []
        )  # If we want to apply cuts (can be done later but fastest at load time)
    ...
    def add_additional_plots(
        self, additional_branches: List[str] | str, exact=False
    ) -> None:
        """
        ...
        for key in self._posterior_ttree.keys():
            ...
            if key in self._ignored_branches:
                continue

            if (
                any(var in key for var in additional_branches) and not exact
            ):  # Not the most efficient but adds variables to our list of variables
    ...
    def ignore_plots(self, ignored_branches: List[str] | str) -> None:
        ...
        for branch in ignored_branches:
            if branch in self._ignored_branches:
                continue
    ...
    def convert_ttree_to_array(self, close_file=True) -> None:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if self._verbose:
                print(
                    f"Using {executor._max_workers} threads and requiring {np.round(self._posterior_ttree.uncompressed_bytes*1e-9,3)} Gb memory"
                )
            ...
            if len(self._plotting_branches) == 0:
                self._ttree_array = self._posterior_ttree.arrays(
                    self._posterior_ttree.keys(),
                    cut=cuts,
                    library="pd",
                ...
                self._ttree_array = self._posterior_ttree.arrays(
                    self._plotting_branches,
                    cut=cuts,
                    library="pd",
                    array_cache=f"{total_memory_needed} b",
                    decompression_executor=executor,
                    interpretation_executor=executor,
                )  # Load in ROOT TTree
            ...
            if self._verbose:
                print(
                    f"Converted TTree to pandas dataframe with {len(self._ttree_array)} elements"
                )
