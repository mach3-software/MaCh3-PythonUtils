...
from MaCh3PythonUtils.machine_learning.tensorflow.tf_autotune_interface import (
    TfAutotuneInterface,
)
from MaCh3PythonUtils.machine_learning.tensorflow.tf_sequential_model import (
    TfSequentialModel,
)
from MaCh3PythonUtils.machine_learning.tensorflow.tf_residual_model import (
    TfResidualModel,
)
from MaCh3PythonUtils.machine_learning.tensorflow.tf_normalizing_flow_model import (
    TfNormalizingFlowModel,
)
...
class MLFactory:
    ...
    def __init__(
        self, input_chain: ChainHandler, prediction_variable: str, plot_name: str
    ):
    ...
    def __setup_package_factory(self, package: str, algorithm: str, **kwargs):
        ...
        if package not in self.__IMPLEMENTED_ALGORITHMS.keys():
            raise ValueError(
                f"{package} not included, currently accepted packages are :\n  \
        ...
        if algorithm not in self.__IMPLEMENTED_ALGORITHMS[package].keys():
            raise ValueError(
                f"{algorithm} not implemented for {package}, currently accepted algorithms for {package} are:\n \
    ...
    def __make_scikit_model(self, algorithm: str, **kwargs) -> SciKitInterface:
        ...
        interface = SciKitInterface(
            self._chain, self._prediction_variable, self._plot_name
        )
        interface.add_model(
            self.__setup_package_factory(
                package="scikit", algorithm=algorithm, **kwargs
            )
        )

        return interface

    def __make_tensorflow_layered_model(
        self, interface: TfManualLayeredInterface, layers: dict
    ) -> TfManualLayeredInterface:
        for layer in layers:
    ...
    def __make_tensorflow_model(self, algorithm: str, **kwargs) -> TfInterface:
        model_func = self.__IMPLEMENTED_ALGORITHMS["tensorflow"].get(
